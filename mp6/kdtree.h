...
class KDTree
{
  ...
  private:
     ...
     */
     int distance(const Point<Dim> & point1, const Point<Dim> & point2) const;

     void build(int currDim, int left, int right);

     int quickSelect(int left, int right, int currDim, int n);

     int partition(int left, int right, int currDim, int n);

     void swap(int x, int y);

     Point<Dim> searchNN(int currDim, const Point<Dim>& query, int left, int right)const;
